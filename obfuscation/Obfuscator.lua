...
-- INSTRUCTIONS:
-- 1. Paste in the code you want to obfuscate into the variable below
local codeToObfuscate=[[
require 'utils'
require 'Vector'
...
-- 3. In the output is the obfuscated version of the script
-- 4. Copy the obfuscated version of the script and paste it into the "obfuscated" variable in Script2
-- 5. Make a Script with the code of Script2 (with your obfuscated code in it), Done!

local s=""
local lasti=0 -- I prefer to have the obfuscated version of the script newline'd every 50 or so obfuscated characters
for i = 1, codeToObfuscate:len() do
  lasti=lasti+1
  local partText=codeToObfuscate:sub(i,i)
  partText=partText:byte()*7
  s=s..partText.."#"
  if lasti>=50 then
    lasti=0
    s=s.."\n"
...
local file = io.open('obfuscated.lua', 'wb')
file:write(s)
file:close()
print('DONE!')
